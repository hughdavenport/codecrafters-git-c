...
#define DEFLATE_CLEAR_BITS(ctx) do { (ctx)->bits.index = 0; (ctx)->bits.data ++; (ctx)->bits.size --; } while (0)
...
uint64_t _deflate_peek_bits_rev(deflate_context *ctx, size_t bits) {
    ...
    assert(bits <= 63);
    uint8_t bytes = (bits + (8 - ctx->bits.index) + 7) / 8;
    switch (bytes) {
        case 0: return 0;
...
int _deflate_fixed_compression_length_extra(deflate_context *ctx) {
    ...
    if (code < 269) {
        uint16_t next = deflate_next_bits(ctx, 1);
        if (next == (uint16_t)EOF) return EOF;
        code = code << 1 | next;
        return 11 + code - (265 << 1);
    } else if (code < 273) {
        // FIXME: Does this need to be _rev
        uint16_t next = deflate_next_bits(ctx, 2);
        if (next == (uint16_t)EOF) return EOF;
        code = code << 2 | next;
        return 19 + code - (269 << 2);
    } else if (code < 277) {
        // FIXME: Does this need to be _rev
        uint16_t next = deflate_next_bits(ctx, 3);
        if (next == (uint16_t)EOF) return EOF;
        code = code << 3 | next;
        return 35 + code - (273 << 3);
    } else if (code < 281) {
        // FIXME: Does this need to be _rev
        uint16_t next = deflate_next_bits(ctx, 4);
        if (next == (uint16_t)EOF) return EOF;
        code = code << 4 | next;
        return 67 + code - (277 << 4);
    } else if (code < 285) {
        // FIXME: Does this need to be _rev
        uint16_t next = deflate_next_bits(ctx, 5);
        if (next == (uint16_t)EOF) return EOF;
        code = code << 5 | next;
    ...
    for (;;) {
        ...
        switch (ctx->state) {
            case ZLIB_HEADER: {
                ...
                if (fdict != 0) UNIMPLENTED("Reading zlib DICTID (see FRC 1950 2.2 Data format)");
                if (flevel != 0) {
